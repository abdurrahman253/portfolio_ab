  ...
  return (
    <motion.div
      ...
      style={{
        ...
        // ✅ FIXED: was zHeight (invalid) — now correct zIndex
        zIndex: 9999,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await emailjs.send(
        EMAILJS_SERVICE_ID,
        EMAILJS_TEMPLATE_ID,
        { from_name: form.name, from_email: form.email, message: form.message, to_email: CONTACT.email, reply_to: form.email },
        EMAILJS_PUBLIC_KEY
      );
      setStatus('idle');
  ...
  return (
    <>
      <style suppressHydrationWarning>{`
        ...
        .contact-title-accent { background: linear-gradient(90deg, #dc2626 0%, #ef4444 40%, #fca5a5 60%, #dc2626 100%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; animation: shimmer-contact 3s linear infinite; }
        /* ✅ FIX: Placeholder color via CSS (was broken with inline --ph variable) */
        .contact-form input::placeholder,
        .contact-form textarea::placeholder { color: #64748b; opacity: 1; }
        /* Input focus ring */
        .contact-form input:focus,
        .contact-form textarea:focus { border-color: rgba(220,38,38,0.4) !important; box-shadow: 0 0 0 3px rgba(220,38,38,0.08); }
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 48px', position: 'relative', zIndex: 1 }} className="contact-container">
          ...
          <div className="contact-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 32 }}>
            ...
            <motion.div className="contact-card" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }}
              ...
              <form onSubmit={handleSubmit} noValidate className="contact-form" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
