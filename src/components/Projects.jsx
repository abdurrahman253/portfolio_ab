...
// ── Data ──────────────────────────────────────────────────────────────────────
// ✅ NEW: Added `role` field to each project for recruiter clarity
const PROJECTS = [
  {
    id: 'scholarstream',
    title: 'ScholarStream',
...
function ProjectPreview({ project }) {
  ...
  return (
    <motion.div onHoverStart={() => setHov(true)} onHoverEnd={() => setHov(false)} style={{ position: 'relative', width: '100%' }}>
      <motion.div
        animate={{ opacity: hov ? 1 : 0 }}
        transition={{ duration: 0.4 }}
        style={{ position: 'absolute', inset: -2, borderRadius: 18, zIndex: 0, background: `linear-gradient(135deg, ${project.accent}40, transparent 60%)`, filter: 'blur(2px)' }}
      />
      <motion.div
        animate={{ y: hov ? -8 : 0, scale: hov ? 1.02 : 1 }}
        transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
        style={{ position: 'relative', zIndex: 1, borderRadius: 16, overflow: 'hidden', boxShadow: hov ? `0 32px 80px rgba(0,0,0,0.65), 0 0 0 1px ${project.accent}40` : `0 16px 48px rgba(0,0,0,0.45), 0 0 0 1px rgba(255,255,255,0.08)`, transition: 'box-shadow 0.4s ease' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 12px', background: 'rgba(10,10,14,0.9)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          {['#ff5f57', '#ffbd2e', '#28c840'].map((c, i) => (
            <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: c }} />
          ))}
          <div style={{ flex: 1, height: 18, borderRadius: 5, marginLeft: 6, background: 'rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', paddingLeft: 8, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', background: project.previewBg }}>
          {err ? (
            <div style={{ aspectRatio: '16/9', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, background: 'rgba(0,0,0,0.4)' }}>
            ...
            <Image src={project.image} alt={project.title} width={720} height={405} style={{ width: '100%', height: 'auto', display: 'block', objectFit: 'cover' }} onError={() => setErr(true)} />
          )}
        </div>
      </motion.div>
    </motion.div>
...
function ProjectRow({ project, index, isDark }) {
  ...
  return (
    <div className={`proj-row ${isOdd ? 'odd' : 'even'}`} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px 64px', alignItems: 'center', padding: '60px 0', position: 'relative' }}>

      {/* Image Block */}
      <motion.div className="proj-image-block" initial={{ opacity: 0, x: isOdd ? -40 : 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} style={{ order: isOdd ? 1 : 2 }}>
        <ProjectPreview project={project} />
      </motion.div>

      {/* Info Block */}
      <motion.div className="proj-info-block" initial={{ opacity: 0, x: isOdd ? 40 : -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.1 }} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', order: isOdd ? 2 : 1 }}>

        {/* Index badge */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 8, background: t.num, fontSize: '0.75rem', fontWeight: 900, color: t.numText, fontFamily: 'monospace' }}>
        ...
        {/* Title */}
        <h3 style={{ margin: '0 0 8px', fontSize: 'clamp(1.5rem, 4vw, 2.1rem)', fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1.1, color: t.title }}>
          {project.title}
        </h3>

        {/* ✅ NEW: Role — shows recruiters exactly what you built */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
          <svg width="12" height="12" fill="none" stroke={t.role} strokeWidth="2" viewBox="0 0 24 24">
            <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/>
            <circle cx="12" cy="7" r="4"/>
          </svg>
          <span style={{ fontSize: '0.72rem', fontWeight: 600, color: t.role, letterSpacing: '0.04em' }}>
            {project.role}
          </span>
        </div>

        <p style={{ margin: '0 0 24px', fontSize: '0.9rem', lineHeight: 1.8, color: t.body, maxWidth: 440 }}>
          {project.description}
        </p>

        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 28 }} className="proj-tags">
          {project.tech.map((tag) => (
            <span key={tag} style={{ padding: '4px 12px', borderRadius: 999, background: t.tagBg, border: `1px solid ${t.tagBord}`, fontSize: '0.68rem', fontWeight: 600, color: t.tagText }}>
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="proj-buttons" style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          <a href={project.github} target="_blank" rel="noopener noreferrer" className="proj-btn-secondary"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 18px', borderRadius: 9, border: `1px solid ${t.btnBord}`, color: t.btnText, textDecoration: 'none', fontSize: '0.76rem', fontWeight: 700, textTransform: 'uppercase', transition: 'all 0.18s' }}>
            <Github size={14} /> GitHub
          </a>
          <a href={project.demo} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 18px', borderRadius: 9, background: project.accent, color: '#fff', textDecoration: 'none', fontSize: '0.76rem', fontWeight: 700, textTransform: 'uppercase', boxShadow: `0 6px 20px ${project.accent}40` }}>
  ...
  return (
    <>
      <style suppressHydrationWarning>{`
        @keyframes proj-shimmer { 0%{background-position:-200% center;} 100%{background-position:200% center;} }
        .proj-title-grad { background: linear-gradient(90deg, #dc2626 0%, #ef4444 40%, #fca5a5 60%, #dc2626 100%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: proj-shimmer 3.5s linear infinite; }
        .proj-btn-secondary:hover { background: rgba(255,255,255,0.06); border-color: rgba(220,38,38,0.3) !important; color: #ef4444 !important; transform: translateY(-1px); }
        @media (max-width: 900px) {
          .proj-container { padding: 0 20px !important; }
          .proj-row { grid-template-columns: 1fr !important; gap: 35px !important; padding: 40px 0 !important; }
          .proj-image-block { order: 1 !important; }
          .proj-info-block { order: 2 !important; text-align: center; align-items: center; }
          .proj-info-block p { margin: 0 auto 24px !important; }
          .proj-info-block > div:nth-child(2) { justify-content: center; }
          .proj-tags, .proj-buttons { justify-content: center; }
        }
        @media (max-width: 480px) {
          .proj-buttons { flex-direction: column; width: 100%; }
          .proj-buttons a { width: 100%; justify-content: center; }
        }
      `}</style>
        ...
        <div className="proj-container" style={{ maxWidth: 1100, margin: '0 auto', padding: '0 48px', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div style={{ textAlign: 'center', marginTop: 40 }}>
            <a href="https://github.com/abdurrahman253" target="_blank" rel="noopener noreferrer"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '12px 24px', borderRadius: 9, border: `1px solid ${divider}`, color: sub, textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600, transition: 'all 0.2s' }}>
